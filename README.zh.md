...
**轻量：** 零依赖，包体积约 160 KB（主要是两张 1024px 图标 PNG；macOS 的 `.icns` 用系统内置 `sips` + `iconutil` 现生成）。只碰桌面侧 —— 不装终端命令、不动 PATH。终端版是独立的 [dsh-launcher](https://github.com/LvienOeria/dsh-launcher) 包，想要什么装什么，绝不捆绑。
